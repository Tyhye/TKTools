...
class MultiListbox(tk.Frame):
    def __init__(self, master, lists):
        super(MultiListbox, self).__init__(master)
        ...
        for l, w in lists:
            ...
            lb = tk.Listbox(frame, width=w, borderwidth=0, selectborderwidth=0,
                relief="flat", exportselection=False, selectmode="extend")
            ...
            lb.bind('<ButtonRelease-1>', lambda e, s=self: s._btn_release_1(e.y))
            lb.bind('<Leave>', lambda e: 'break', add=True)
            lb.bind('<B2-Motion>', lambda e, s=self: s._b2motion(e.x, e.y), add=True)
            lb.bind('<Button-2>', lambda e, s=self: s._button2(e.x, e.y), add=True)
            # lb.send()
        self.last_row = -1
        frame = tk.Frame(self); frame.pack(side="left", fill="y")
        tk.Label(frame, borderwidth=1, relief="raised").pack(fill="x")
        self.sb = tk.Scrollbar(frame, orient="vertical", command=self._scroll)
